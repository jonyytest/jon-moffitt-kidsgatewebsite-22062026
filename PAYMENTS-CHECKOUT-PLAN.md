...
> the small VPN minority), and run referrals through **Stripe promotion codes plus an
...
We should not build a custom card form. This is not a matter of skill or effort. A
homemade checkout is simply worse on every axis that matters.
...
- ✅ Feels completely like our site, which solves the trust problem entirely
...
### How to pass it
1. **URL parameters.** Simplest, and it has a useful side effect: the URL becomes
   *shareable and bookmarkable*, so a parent can save or resume their configuration.
   ...
   ```
2. **Server-side cart token.** The site POSTs the selection and gets back a short token
...
- ✅ Fewest steps before payment, so the highest conversion
- ✅ Their plan choice flows straight into checkout with nothing lost
- ⚠️ Child profiles and subject assignment happen *after* payment (fine, see §6)
...
Plan builder **and** embedded checkout on one page. Adjust children and subjects on the
left, the total and payment form update on the right, sign-up fields inline.

- ✅ Nothing to carry over, because it never leaves the page
...
target, and ship **Option 1** first if we need to launch sooner. Options 1 and 3 both
...
this: `client_reference_id` and metadata) and store the Stripe customer and subscription
IDs on the account. That two-way link is what makes everything else work.
...
Both of those assume **identical units × quantity**. Our children are not identical (one
may do two subjects, another one), and the "most subjects pays full" rule is a business
decision, not quantity maths. So: flat-rate prices, assembled by our code.

### The activation fee
- Charged **once per child** while continuously enrolled.
- **Carries over** monthly → annual (no re-charge).
- **Re-applies** only if an account lapses or cancels and later rejoins.
...
- **We already cater to it.** The footer region selector lets anyone correct the guess
  in one click. VPNs are an argument *for having a manual override*, which we have, not
  for abandoning geolocation.
...
gets redirected, and regional pages index poorly. It's also irritating when it guesses
wrong.

**So: geo-*suggest*, not geo-*force*.**
1. Detect location, then set a **smart default** for language and currency.
...
from datacentre and VPN ranges. We could route flagged visitors to a neutral
...
| Effort to convert | Must re-engage via email or in-app | Converts by default |
...
Reasoning:
- The conversion difference is not marginal. It is usually severalfold, and that
  difference determines whether paid acquisition is viable at all.
...
The goal: a promoter shares a code or link, the buyer gets a discount, and the promoter
gets paid for the signup.

### What Stripe does natively
- **Coupons** (the discount) and **Promotion codes** (customer-facing codes like
  `SARAH20`) are fully built in, including an "add promotion code" field in Checkout. ✅
...
- **Link-based attribution via cookies**, which catches people who click through without
  typing a code (a large share of them)
- Automatic recurring-commission calculation (e.g. a % of every renewal, not just signup)
...
Referral programmes are deceptively complex. The hard parts aren't the discount. They are
...
**Recommended combination:** a unique **referral link** for tracking (cookie-based)
**plus** a memorable **promo code** for the buyer's discount and for offline or
word-of-mouth sharing. The link catches the clicks; the code catches the conversations.
...
7. **Indonesia:** Stripe is limited there, so which local gateway (Midtrans/Xendit), and
...
9. **Tax and GST:** do we use Stripe Tax? Which markets are we registered in?
...
15. Data residency for customer and payment records.
...
2. **Decide web vs in-app** subscriptions (item 3). This is a commercial decision, not a
   technical one.
...
> subjects for the right children, and **we need to confirm that endpoint exists**.
> Referral discounts use Stripe promotion codes, with an affiliate add-on handling
> promoter attribution and commission payouts. Geolocation sets a smart regional default
> with a dismissable suggestion banner, never a forced redirect, and the manual region
> selector stays for the small VPN minority, with the billing address at checkout being
> the real authority on where the customer is.

---

*Prepared as a planning document. Payment platform capabilities, fees and country
availability change, so confirm current details with Stripe before committing.*
